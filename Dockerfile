...
FROM mcr.microsoft.com/dotnet/runtime:${DOTNET_RUNTIME_VERSION}-nanoserver-${WINDOWS_VERSION}
...
RUN setx /M PATH "%PATH%;%PYTHONPATH%" && \
    setx /M PYTHONPATH "%PYTHONPATH%" && \
    setx /M PIP_CACHE_DIR "C:\Users\ContainerUser\AppData\Local\pip\Cache" && \
